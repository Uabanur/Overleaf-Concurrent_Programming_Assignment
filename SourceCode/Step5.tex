\section*{Step 5 EXTRA CarControl}
\addcontentsline{toc}{section}{Step 5 EXTRA CarControl}
   ...
   public void run() {
    try {
      ...
      while (true) { 
        ...
        if((no <= 4 && newpos.row == 9 && newpos.col == 4) ||
           (no >= 5 && newpos.row == 10 && newpos.col == 0) )
        {
          cr.bridge.leave();
        }
...
class Alley {
  short carsTop = 0;
  short carsBottom= 0;
  int topCarsThrough = 0;
  int bottomCarsThrough = 0;

  public synchronized void enter(int no) throws Exception
  {
    if(no<=4) //Top going down
    {   
      while(carsBottom>0||topCarsThrough>bottomCarsThrough+3) 
        wait();
      carsTop++;
      topCarsThrough++;
    }
    ...
    { 
      while(carsTop>0||bottomCarsThrough>topCarsThrough+3)
      wait();
      carsBottom++;
      bottomCarsThrough++;
    }
